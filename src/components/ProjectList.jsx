...
function TodoList() {
  ...
  return (
    <div className="todo-list" style={{ backgroundColor: 'darkblue', padding: '20px' }}> {/* Added background color and padding */}
      <div className="add-task" style={{ marginBottom: '30px', display: 'flex', alignItems: 'center' }}>
        <input
      ...
      <div className="task-cards">
        {tasks.map(task => (
          <Card key={task.id} className={`task-card ${task.status === 'completed' ? 'completed' : ''}`} style={{ marginBottom: '50px' }}>
            <CardContent>
              <Link to="/TodoList" style={{ textDecoration: 'none', color: 'inherit' }}> {/* Link to TodoList page with custom styles */}
                {/* Content of the card */}
